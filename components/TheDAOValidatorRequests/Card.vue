  ...
  >
    <div class="overflow-hidden w-full h-[200px] rounded-xl rounded-b-none">
      <img
        ...
        class="w-full h-full rounded-xl object-contain ml-auto hover:scale-105 bg-white rounded-b-none"
      />
    </div>
      ...
      <div class="flex flex-col justify-between gap-2">
        <div class="flex justify-between">
          ...
          <span class="text-xs text-Gray-b4 text-end flex items-center">{{
            convertDate(state.proposalDetail.endDate)
          }}</span>
        </div>
        <div class="flex gap-2 items-center">
          ...
          />
          <h5 class="lg:text-base text-sm text-Gray-b4 line-clamp-1">
            {{ shortenAddress(state.proposalDetail.creator) }}
          </h5>
        </div>
      </div>
      <div class="flex justify-between items-center mt-4">
        <div class="flex items-center bg-Gray-b1 rounded-3xl">
          <div
            class="w-14 h-8 flex items-center justify-center text-Gray-b3 md:text-base text-sm gap-1"
          >
            <i class="isax isax-like-1 text-xl text-green-800" />
            {{ state.yesVotes }}
          </div>
          <div class="h-5 bg-Gray-b3 w-[2px] rounded-full"></div>
          <div
            class="w-14 flex items-center justify-center text-Gray-b3 md:text-base text-sm gap-1"
          >
